...
function loadChar(name, x, y)
	...
	for line in io.lines(player.name..".tsv") do -- Iterate through each line of player.tsv (tab separated values)
		if #line > 0 then
			src_x, src_y, src_w, src_h, dst_x, dst_y, dst_w, dst_h, spr_group_id, spr_img_no= line:match("(%d+)\t(%d+)\t(%d+)\t(%d+)\t(%d+)\t(%d+)\t(%d+)\t(%d+)\t(%d+)%D(%d+)")
		...
		if player.anims[player.state] == nil then
			...
			if player.anims[player.state][player.frame_no] == nil then
				...
				if anim ~= nil then
					...
					if dt ~= nil then
						player.sprites:add(love.graphics.newQuad(dt[1], dt[2], dt[3], dt[4], player.atlas_img_w, player.atlas_img_h), player.x + dt[5] + anim.spr_x, player.y + dt[6] + anim.spr_y)
					else
